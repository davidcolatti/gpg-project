	...
	return (
		<div className="MainArea">
			<Popup modal trigger={<span className="trigger" />}>
					{(close) => (
					<PostModal selectedPost={selectedPost}/>
					)}
			</Popup>
			...
			<div className="main-bottom-div">
				<div className="post-list-title">
					...
					<span className="right-title">Total posts {renderedPosts?.length}</span>
				</div>
                <div className="post-list segment ui middle aligned divided list">
                    <PostList setSelectedPost={setSelectedPost} toggleTrigger={toggleTrigger} renderedPosts={renderedPosts} setPosts={setPosts} setRenderedPosts={setRenderedPosts} {...props} />
                </div>
				<div className="posts-button-div">
					{renderedPosts?.length < posts?.length ? <button onClick={loadMorePostsBtn} className="ui button">Load More Posts</button> : ''}
				</div>
			</div>
		</div>
	);
